...
| **Conversation flow** | `POST /create-conversation-flow`, `GET /get-conversation-flow/{id}`, `GET /v2/list-conversation-flows`, `PATCH /update-conversation-flow/{id}`, `DELETE /delete-conversation-flow/{id}` | CRUD full and field-lossless (round-trip proven against real Retell fixtures in `backend/tests/fixtures/retell_flows/`). The voice worker now **executes** flow-backed agents on live calls: node types `conversation`, `subagent`, `branch`, `function`, `transfer_call`, `end`, `extract_dynamic_variables` run; `press_digit`, `agent_swap`, `sms`, `mcp`, `code`, `component`, `bridge_transfer`, `cancel_transfer` are rejected at call start — before the greeting, naming the offending node — rather than reached mid-call. See `docs/ARCHITECTURE.md` for the execution model. No dashboard editor yet (flows are authored via the API only), and flow agents cannot yet be exercised through the Simulation suite (below) |
...
- `POST /generate-test-case-definitions` has no Retell equivalent: it drafts
  ...
  live call does.
- The dashboard's Test Audio / Simulation panel is gated on an agent having a
  Retell LLM (`llm !== null`), so it currently offers no way to test a
  conversation-flow agent from the UI even though the worker can run one —
  this is a dashboard gap, not a runtime limitation, and belongs to the
  editor work rather than flow execution.
