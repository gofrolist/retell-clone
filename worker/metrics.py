"""Prometheus metrics for the Architeq voice worker.

Exposed on ``:9090`` (override with ``ARCHITEQ_METRICS_PORT``). Series names
...
logger = logging.getLogger("architeq-worker.metrics")

JOBS_TOTAL = Counter(
    "architeq_worker_jobs_total",
    "Voice agent jobs handled by this worker",
    ["direction"],
)

TOOL_CALLS_TOTAL = Counter(
    "architeq_tool_calls_total",
...
def ensure_server(port: int | None = None) -> None:
    ...
    if _server_started:
        return
    if port is None:
        port = int(os.getenv("ARCHITEQ_METRICS_PORT", "9090"))
    try:
